...
////ensures value(R, U) == value(old(R), old(U)) + n;
...
////ensures value(R, U) == value(old(R), old(U));
...
{ 
 assume inv(R,U); 
 assume (_n0 > 0  && _id0 >= 0 && _id0 < replicas&& local_rights(_id0, R, U) >= _n0); 
  assume (_n1 > 0  && _from1 >= 0 && _from1 < replicas&& _to1 >= 0 && _to1 < replicas && _from1 != _to1&& local_rights(_from1, R, U) >= _n1);
 call decrement(_id0,_n0);
