...
axiom(forall bids:Bid :: 0 <= get_winner(bids) && get_winner(bids) < nb && bids[get_winner(bids)][placed] && is_highest_bid(bids, get_winner(bids)));
...
@gteq
function gteq(status1:bv2, winner1:BidId, bids1:Bid, token1:[ReplicaId]bool, right1:ReplicaId, status2:bv2, winner2:BidId, bids2:Bid, token2:[ReplicaId]bool, right2:ReplicaId) returns(bool)
{
    bv2uge(status1, status2) && ((winner1 != null) || (winner2 == null)) && (forall b:BidId :: (0 <= b && b < nb) ==> (bids1[b][placed] || !bids2[b][placed])) && (forall r:ReplicaId :: (0 <= r && r < nr) ==> (!token1[r] || token2[r]))
}

@invariant
function inv(status:bv2, winner:BidId, bids:Bid, token:[ReplicaId]bool, right:ReplicaId) returns(bool)
{
...
@merge
procedure merge(status1:bv2, winner1:BidId, bids1:Bid, token1:[ReplicaId]bool, right1:ReplicaId)
...
requires ((right == me) ==> (winner == winner1 || winner1 == null));
requires ((forall b:BidId :: (token_released(token) && 0 <= b && b < nb && !bids[b][placed]) ==> !bids1[b][placed]));
modifies status, winner, bids, token, right;
