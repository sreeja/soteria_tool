...
const _to:ReplicaID;
const _value:int;
procedure stability_releaseCredit()
modifies balances;
ensures 0 <= client && client < nc && 0 <= to && to < nr && value > 0 && balances[client][me][d] - balances[client][me][w] >= value + min;
{
assume inv(balances);
assume 0 <= client && client < nc && 0 <= to && to < nr && value > 0 && balances[client][me][d] - balances[client][me][w] >= value + min;
assume 0 <= _client && _client < nc;
assume 0 <= _to && _to < nr;
assume _value > 0;
assume balances[_client][me][d] - balances[_client][me][w] >= _value + min;
call releaseCredit(_client, _to, _value);
