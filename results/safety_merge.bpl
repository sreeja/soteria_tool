...
const _bids1:Bid;
procedure safety_merge()
modifies status, winner, bids;
ensures (forall b:BidId :: (0 <= b && b < nb && bids[b][placed]) ==> (bv2uge(status, active) && bids[b][amount] > 0))
    && (bv2uge(active, status) ==> winner == null)
    && (status == closed ==> (exists b:BidId :: 0 <= b && b < nb && b != null && bids[b][placed] && winner == b && is_highest_bid(bids, winner)));
{
assume inv(status,winner,bids);
assume inv(_status1,_winner1,_bids1);
