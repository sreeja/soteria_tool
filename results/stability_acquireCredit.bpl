...
//  }
//}
const balances1:[Client][ReplicaID][Txn]int;
procedure acquireCredit(client:Client, from:ReplicaID, value:int)
modifies balances;
requires (0 <= client && client < nc);
requires (0 <= from && from < nr);
...
const _value:int;
procedure stability_acquireCredit()
modifies balances;
ensures (forall c:Client, r:ReplicaID :: (0 <= c && c < nc && 0 <= r && r < nr) ==> (max(balances[c][r][d], balances1[c][r][d]) >= max(balances[c][r][w], balances1[c][r][w]) + min));
{
assume inv(balances);
assume (forall c:Client, r:ReplicaID :: (0 <= c && c < nc && 0 <= r && r < nr) ==> (max(balances[c][r][d], balances1[c][r][d]) >= max(balances[c][r][w], balances1[c][r][w]) + min));
