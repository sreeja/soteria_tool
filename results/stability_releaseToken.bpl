...
procedure stability_releaseToken()
modifies token;
ensures (forall b:BidId :: (0 <= b && b < nb) ==> (bids[b][amount] == bids1[b][amount]));
ensures ((status == closed) ==> (is_highest_bid(bids1, winner) && is_highest_bid(bids, winner)));
ensures ((status1 == closed) ==> (is_highest_bid(bids, winner1) && is_highest_bid(bids1, winner1)));
ensures (token[me] ==> token1[me]);
ensures ((token_released(token)) ==> (winner == winner1 || winner1 == null));
ensures (!token_released(token) ==> (winner == null && winner1 == null));
ensures ((forall b:BidId :: (token_released(token) && 0 <= b && b < nb && !bids[b][placed]) ==> !bids1[b][placed]));
ensures ((forall b:BidId :: ((forall r:ReplicaId :: (0 <= r && r < nr && r != me) ==> !token[r]) && 0 <= b && b < nb && !bids[b][placed]) ==> !bids1[b][placed]));
{
assume inv(status,winner,bids,token) && inv(status1,winner1,bids1,token1);
assume (forall b:BidId :: (0 <= b && b < nb) ==> (bids[b][amount] == bids1[b][amount]));
assume ((status == closed) ==> (is_highest_bid(bids1, winner) && is_highest_bid(bids, winner)));
assume ((status1 == closed) ==> (is_highest_bid(bids, winner1) && is_highest_bid(bids1, winner1)));
assume (token[me] ==> token1[me]);
assume ((token_released(token)) ==> (winner == winner1 || winner1 == null));
assume (!token_released(token) ==> (winner == null && winner1 == null));
assume ((forall b:BidId :: (token_released(token) && 0 <= b && b < nb && !bids[b][placed]) ==> !bids1[b][placed]));
assume ((forall b:BidId :: ((forall r:ReplicaId :: (0 <= r && r < nr && r != me) ==> !token[r]) && 0 <= b && b < nb && !bids[b][placed]) ==> !bids1[b][placed]));
assume (token[me]);
call releaseToken();
