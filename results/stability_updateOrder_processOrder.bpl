...
const _qty0:int;
const _id1:OrderId;
const _storeloc1:ReplicaID;
procedure stability_test() 
 modifies UserOrders; 
 modifies UserOrders,BookStore; 
{ 
 assume inv(BookStore,UserOrders,Users); 
 assume (_qty0 >= 0 && UserOrders[_id0][created] == true && UserOrders[_id0][placed] == false && UserOrders[_id0][user] == _usr0); 
  assume (UserOrders[_id1][placed] == true && UserOrders[_id1][processed] == false && UserOrders[_id1][cancelled] == false&& (forall b:ISBN :: BookStore[b][P][_storeloc1] - BookStore[b][N][_storeloc1] >= UserOrders[_id1][items][b]));
 call updateOrder(_id0,_usr0,_book0,_qty0);
