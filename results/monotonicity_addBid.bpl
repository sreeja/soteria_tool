...
const _value:int;
procedure monotonicity_addBid()
modifies bids, bid_auction, bid_amount;
ensures gteq(auctions,auction_winner,auction_prep,auction_active,auction_closed,bids,bid_auction,bid_amount,old(auctions),old(auction_winner),old(auction_prep),old(auction_active),old(auction_closed),old(bids),old(bid_auction),old(bid_amount));
{
