...
modifies bids;
ensures (winner == winner1) || (winner == null) || (winner1 == null);
...
{
assume inv(status,winner,bids) && inv(status1,winner1,bids1);
...
assume 0 <= _bid_identifier && _bid_identifier < nb && !bids[_bid_identifier][placed] && bids[_bid_identifier][amount] == _value;
assume status == active && winner == null;
assume _value > 0;
call placeBid(_bid_identifier, _value);
