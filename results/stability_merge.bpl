...
const _status1:bv2;
const _winner1:BidId;
const _bids1:Bid;
procedure stability_merge()
modifies status, winner, bids;
ensures (winner == _winner1) || (winner == null) || (_winner1 == null);
ensures (forall b:BidId :: (0 <= b && b < nb) ==> (bids[b][amount] == _bids1[b][amount]));
ensures ((status == closed) ==> (is_highest_bid(_bids1, winner) && is_highest_bid(bids, winner)));
ensures ((_status1 == closed) ==> (is_highest_bid(bids, _winner1) && is_highest_bid(_bids1, _winner1)));
{
assume inv(status,winner,bids) && inv(_status1,_winner1,_bids1);
assume (winner == winner1) || (winner == null) || (winner1 == null);
...
assume (winner == _winner1) || (winner == null) || (_winner1 == null);
assume (forall b:BidId :: (0 <= b && b < nb) ==> (bids[b][amount] == _bids1[b][amount]));
assume ((status == closed) ==> (is_highest_bid(_bids1, winner) && is_highest_bid(bids, winner)));
assume ((_status1 == closed) ==> (is_highest_bid(bids, _winner1) && is_highest_bid(_bids1, _winner1)));
call merge(_status1, _winner1, _bids1);
