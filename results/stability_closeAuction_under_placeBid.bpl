...
assume 0 <= _bid_identifier && _bid_identifier < nb && !bids[_bid_identifier][placed] && bids[_bid_identifier][auction] == none && bids[_bid_identifier][amount] == 0;
assume 0 <= _auction_identifier && _auction_identifier < na && auctions[_auction_identifier][reg] && auctions[_auction_identifier][status] == active && auctions[_auction_identifier][winner] == null;
assume _value > 0;
assume !(exists b:BidId :: 0 <= b && b < nb && b != _bid_identifier && bids[b][placed] && bids[b][auction] == _auction_identifier && bids[b][amount] == _value);
call placeBid(_bid_identifier, _auction_identifier, _value);
