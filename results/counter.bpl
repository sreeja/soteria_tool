...
{
  (forall r:int :: (0 <= r && r < replicas) ==> (U[r] == max(newU[r], oldU[r])))
}	
function update2dmatrix(newR:matrix2d, oldR:matrix2d, R:matrix2d) returns(bool)
{
  (forall r1, r2:int :: (0 <= r1 && r1 < replicas && 0 <= r2 && r2 < replicas) ==> (R[r1][r2] == max(newR[r1][r2], oldR[r1][r2])))
}	
...
ensures update2dmatrix(newR, old(R), R) && update1dmatrix(newU, old(U), U);
{
  ...
  j := 0;
  while (i < replicas)
  invariant (0 <= i && i <= replicas);
  invariant (forall h, k:int :: (0 <= h && h < i && 0 <= k && k < replicas) ==> R[h][k] == max(old(R)[h][k], R[h][k]));
  invariant (forall h, k:int :: (i <= h && h < replicas && 0 <= k && k < replicas) ==> R[h][k] == old(R)[h][k]);
  invariant (forall h:int :: (0 <= h && h < i) ==> U[h] == max(old(U)[h], U[h]));
  invariant (forall h:int :: (i <= h && h < replicas) ==> U[h] == old(U)[h]);
  {
    U[i] := max(tempU[i], newU[i]);
    j := 0;
    while (j < replicas)
    invariant (0 <= j && j <= replicas);
    invariant (i == old(i));
    invariant (forall k:int :: (0 <= k && k < j) ==> R[i][k] == max(old(R)[i][k], newR[i][k]));
    invariant (forall k:int :: (j <= k && k < replicas) ==> R[i][k] == old(R)[i][k]);
    invariant (forall h, k:int :: (0 <= h && h < i && 0 <= k && k < replicas) ==> R[h][k] == max(old(R)[h][k], newR[h][k]));
    invariant (forall h, k:int :: (i < h && h < replicas && 0 <= k && k < replicas) ==> R[h][k] == old(R)[h][k]);
    {
      R[i][j] := max(tempR[i][j], newR[i][i]);
      j := j + 1;
    }
    i := i + 1;
  }}
